    ...
    class Thresh
    {
        ...
        private void Menu()
        {
            try
            {
                var Combo = new Menu("Combo", "Combo");
                {
                    ...
                    Combo.AddItem(new MenuItem("Thresh_TUse_Q", "Grab the bitch").SetValue(new KeyBind('T', KeyBindType.Press)));
                ...
                var Misc = new Menu("Misc", "Misc");
                {
                    ...
                    Misc.AddItem(new MenuItem("Thresh_GrabDash", "Grab to dashing enemy use Q").SetValue(true));
        ...
        private static void OnGameUpdate(EventArgs args)
        {
            try
            {
                ...
                // Set Target
                var QTarget = TargetSelector.GetTarget(_Q.Range, TargetSelector.DamageType.Magical);
                var WTarget = ObjectManager.Get<Obj_AI_Hero>().OrderByDescending(x => x.Health).FirstOrDefault(x => x.IsAlly && !x.IsDead && !x.IsMe && x.HealthPercent < _MainMenu.Item("Thresh_LenternHP").GetValue<Slider>().Value && x.Distance(Player) < 1600);
               ...
               if (_MainMenu.Item("Thresh_TUse_Q").GetValue<KeyBind>().Active)
                {
                    if (_MainMenu.Item("Thresh_GrabSelect" + QTarget.ChampionName).GetValue<StringList>().SelectedIndex != 1
                ...
                if (_MainMenu.Item("Thresh_GrabDash").GetValue<bool>() && _Q.IsReady())
